...
#include <algorithm> // std::min, std::max, std:max_element std:min_element
using namespace std;
...
void pressure_solver(double** p, double SOR,  double** F_n, double** G_n, int nx, int ny, double dt, double dx, double dy, int ext_it) {
  ...
  for (int iteration=1; iteration <= 200; iteration++) {
    if (iteration % 4 == 1 || iteration % 4 == 2) {
      //if (ext_it % 2 == 0) {
      for (int i=1; i <= nx-2; i++) {
	for (int j=1; j <= ny-2; j++) {
	...
	for (int j=1; j <= ny-2; j++) {
      ...
      double outcome = sqrt(tmp/double((nx-2)*(ny-2)));
      double percent_diff_norm = abs((outcome - last_outcome)/outcome)*100;
      double percent_diff_maxNorm = abs((max_norm - last_maxNorm)/max_norm)*100;
      for (int a = n-1; a >= 1; a--) {
	buff_norm[a] = buff_norm[a-1];
	buff_max[a] = buff_max[a-1];
      }
      buff_norm[0] = percent_diff_norm;
      buff_max[0] = percent_diff_maxNorm;
      double mean_norm = mean(buff_norm, n);
      double mean_maxNorm = mean(buff_max, n);
      cout << iteration << "\t" << outcome  << "\t" << max_norm << "\t" << mean_norm << "\t" << mean_maxNorm << "\t" << mean_norm*mean_maxNorm << "\n";
      if (mean_norm * mean_maxNorm < 1) {
	break;
      }
      last_outcome = outcome;
      last_maxNorm = max_norm;
    }
  }
  cout << "------------------\n";
}
...
double variable_dt(double tau, double nx, double ny, double Re, double dx, double dy, double** u, double** v) {
  ...
  return tau * min((Re/2)/(1/pow(dx,2) + 1/pow(dy,2)), min(dx/u_max, dy/v_max));
}
