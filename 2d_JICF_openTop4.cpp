...
void paraview2D(string fileName, double** u_new, double** v_new, double** p, double** phi_new, int nx, int ny, double dx, double dy, int precision, double t) {
  ofstream myfile;
  myfile.open(fileName);
...
int main() {
  ...
  string f_name = "vtk_files/2dJICF_openTop2/2dJICF_openTop2_";
  //string name_prefix = "checkpoints/Validation_2DChannelFlow/";
  const int precision = 10;
  const int save_precision = 6;
  paraview2D(f_name + to_string(1) + ".vtk", u_new, v_new, p, phi_new, nx, ny, dx, dy, precision, 0.0);
  ...
  for (int it=1; it <= 100000; it++) { //100000
    ...
    if (it % 1 == 0) { //20
      //cout << it << "\n";
      if (it % 100 == 0) { //1000
	paraview2D(f_name + to_string(it) + ".vtk", u_new, v_new, p, phi_new, nx, ny, dx, dy, precision, t);
      }
      /*
      if (it % 200 == 0) {
