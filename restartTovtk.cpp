...
int main() {
  ...
  string f_name = "/media/sf_Shared/shared_vtk/3DJICF_full/JICF_";
  ...
  const int start_steady_at = 500; // 20000 just picked a number
  const int end_steady_at = 13000;
  const int step = 500;
  ...
  for (int it = start_steady_at; it <= end_steady_at; it += step) {
    ...
    paraview3D(f_name + to_string(it) + ".vtk", u, v, w, p, phi, nx, ny, nz, dx, dy, dz, save_precision, t);
  }
