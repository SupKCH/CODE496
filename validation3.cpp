...
int main() {
  ...
  const int ny = 800; // increases by 4 times, previously =50
  const double dy = (1.0/(double)ny); // becomes smaller by 4 times
  const double dx = 1.0*dy; // increases by 4 times, previously =4.0*dy // changed back to 4.0 from 16.0
  const double dt = 0.0001; // Previously 0.005
  ...
  paraview(filename_prefix + "Fn_" + to_string(1) + ".vtk", F_n, nx, ny, dx, dy, "u");
  paraview(filename_prefix + "Gn_" + to_string(1) + ".vtk", G_n, nx, ny, dx, dy, "v");
  cout << "1\n";
