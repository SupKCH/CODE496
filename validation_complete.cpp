...
void F_calculation(double** F_n, double** u, double** v, int nx, int ny, double dt, double Re, double dx, double dy) {
  for (int i=1; i <= nx-3; i++) {
    for (int j=1; j <= ny-2; j++) {
      F_n[i][j] = u[i][j] + dt*( \
				(d2u_dx2(u,i,j,dx) + d2u_dy2(u,i,j,dy))/Re - \
				 du2_dx(u,i,j,dx) - duv_dy(u,v,i,j,dy) + 0.0 );
    }
  }
  for (int j=1; j <= ny-2; j++) {
    F_n[0][j] = u[0][j];
...
void G_calculation(double** G_n, double** u, double** v, int nx, int ny, double dt, double Re, double dx, double dy) {
  for (int i=1; i <= nx-2; i++) {
    for (int j=1; j <= ny-3; j++) {
      G_n[i][j] = v[i][j] + dt*( (d2v_dx2(v,i,j,dx) + d2v_dy2(v,i,j,dy))/Re - \
				 duv_dx(u,v,i,j,dx) - dv2_dy(v,i,j,dy) + 0.0 );
    }
  }
  for (int i=1; i <= nx-2; i++) {
    G_n[i][0] = v[i][0];
...
void pressure_cal(double** p, double SOR,  double** F_n, double** G_n, int nx, int ny, double dt, double dx, double dy, int i, int j) {
  ...
  p[i][j] = \
    (1-SOR) * p[i][j] + \
    SOR/((eE(i, nx) + eW(i))/(dx*dx) + (eN(j, ny) + eS(j))/(dy*dy)) * \
...
void pressure_solver(double** p, double SOR,  double** F_n, double** G_n, int nx, int ny, double dt, double dx, double dy, int ext_it) {
  ...
  for (int iteration=1; iteration <= iteration_limit; iteration++) {
	...
	for (int j=1; j <= ny-2; j++) {
      ...
      double norm = sqrt(tmp/double((nx-2)*(ny-2)));
      cout << iteration << "\t" << norm << "\t" << max_norm << "\n";
      if (iteration >= 10 && norm <= 0.1) break;
    }
  }
  cout << "------------------\n";
}
...
void paraview2D(string fileName, double** u_new, double** v_new, double** p, double** phi_new, int nx, int ny, double dx, double dy, int precision) {
  ...
  for (int j = 0; j <= ny-1; j++) {
    for (int i = 0; i <= nx-1; i++) {
      myfile << setprecision(precision) << pow(u_new[i][j], 2) + pow(v_new[i][j], 2) << "\n";
    }
  }
...
void save_time(double t, string name_prefix, int iteration, int nx, int ny) {
  ofstream myfileO;
  myfileO.open(name_prefix + to_string(nx) + "x" + to_string(ny) + "_t_" + to_string(iteration) + ".dat");
  myfileO << t << endl;
  myfileO.close();
}
...
double variable_dt(double tau, int nx, int ny, double Re, double dx, double dy, double** u, double** v) {
  ...
  for (int i=0; i <= nx-1; i++) {
    for (int j=0; j <= ny-1; j++) {
      ...
      if (v[i][j] > v_max) v_max = v[i][j];
    }
  }
  return tau * min( (Re/2)/(1/pow(dx,2) + 1/pow(dy,2)), \
		    min(dx/u_max, dy/v_max));
}
...
int main() {
  ...
  int readAt_iteration = 0;
  // **** ------------------------ ****

  //string f_name = "vtk_files/validBest_alt/validBestAlt_";
  string f_name = "/media/sf_Shared/shared_vtk/2DValidationChannelFlow/validation_";
  ...
  readAt_iteration = 0;
    ...
    read_restartfile(u, name_prefix, "u", readAt_iteration, nx, ny);
    read_restartfile(v, name_prefix, "v", readAt_iteration, nx, ny);
    read_restartfile(p, name_prefix, "p", readAt_iteration, nx, ny);
    read_restartfile(phi, name_prefix, "phi", readAt_iteration, nx, ny);
    t = load_time(name_prefix, readAt_iteration, nx, ny);
  }
  ...
  for (int it = readAt_iteration + 1; it <= 34000; it++) { //100000
    ...
    if (it % 500 == 0) {
      ...
      save_time(t, name_prefix, it, nx, ny);
    }
  }
}
