...
int main() {
  ...
  for (int it=1; it <= 100000; it++) {
    ...
    if (it % 20 == 0) {
      cout << it << "\n";
      if (it % 1000 == 0) {
	paraview(filename_prefix + "u_" + to_string(it) + ".vtk", u_new, nx, ny, dx, dy, "u");
	paraview(filename_prefix + "v_" + to_string(it) + ".vtk", v_new, nx, ny, dx, dy, "v");
	paraview(filename_prefix + "p_" + to_string(it) + ".vtk", p, nx, ny, dx, dy, "p");
      }
    }
  }
  /*
