...
int main() {
  ...
  string f_name = "vtk_files/valid_mpi/valid_mpi_";
  ...
  */
  ...
  double **u1;
  double **v1;
  G_n1 = (double **) malloc (nx * sizeof(double));
  for (int i = 0; i < nx; i++) {
    G_n1[i] = (double *) malloc (ny * sizeof(double));
  }
  ...
  v1 = (double **) malloc (nx * sizeof(double));
  for (int i = 0; i < nx; i++) {
    v1[i] = (double *) malloc (ny * sizeof(double));
  }
  */
  ...
  */
  ...
  for (int it=1; it <= 100000; it++) { //100000
    cout << "Hello0\n";
    if (rank == 0) {
      //MPI_Send(G_n, nx*ny, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
      MPI_Send(u, nx*ny, MPI_DOUBLE, 1, 1, MPI_COMM_WORLD);
      MPI_Send(v, nx*ny, MPI_DOUBLE, 1, 2, MPI_COMM_WORLD);
      F_calculation(F_n, u, v, nx, ny, dt, Re, dx, dy);
      cout << "Hello0.5\n";
      MPI_Recv(&(G_n[0][0]), nx*ny, MPI_DOUBLE, 1, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else if (rank == 1) {
      cout << "Hello1\n";
      //MPI_Recv(G_n, nx*ny, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      cout << "Hello2\n";
      cout << u[0] << "\n";
      cout << u[0][0] << "\n";
      MPI_Recv(&(u[0][0]), nx*ny, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(&(v[0][0]), nx*ny, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      //quick_visualize(u, nx, ny);
      cout << u[0][0] << "\n";
      cout << "Hello3\n";
      //nothing(u);
      cout << "Hello3.1\n";
      ...
      cout << "Hello3.5\n";
      MPI_Send(G_n, nx*ny, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD);
    }
    ...
    if (rank == 0) {
      MPI_Send(v_new, nx*ny, MPI_DOUBLE, 1, 4, MPI_COMM_WORLD);
      MPI_Send(p, nx*ny, MPI_DOUBLE, 1, 5, MPI_COMM_WORLD);
      u_calculation(u_new, u, F_n, p, dt, dx, nx, ny);
      MPI_Recv(v_new, nx*ny, MPI_DOUBLE, 1, 6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(v, nx*ny, MPI_DOUBLE, 1, 7, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else if (rank == 1) {
      MPI_Recv(v_new, nx*ny, MPI_DOUBLE, 0, 4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(p, nx*ny, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      v_calculation(v_new, v, G_n, p, dt, dy, nx, ny);
      MPI_Send(v_new, nx*ny, MPI_DOUBLE, 0, 6, MPI_COMM_WORLD);
      MPI_Send(v, nx*ny, MPI_DOUBLE, 0, 7, MPI_COMM_WORLD);
    }
    //MPI_Barrier(MPI_COMM_WORLD);
